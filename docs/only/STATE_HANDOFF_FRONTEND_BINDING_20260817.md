...
## 十一、前端治理第二轮 + 管道/深层链可读化（2026-08-18, 已提交本地）

### 完成（全部实测）
- **画像修复**: API 维度键为单字母（O/C/E/A/N/NC/CS/DK/MS/CL）, 补中文映射。
- **路由切换修复**: 后端补 PUT /v6/router/modes + kernel_set_router_mode
  （持久化 + 存根同步）; 实测 active 切换生效。
- **网关用量时间轴**: /v6/gateway/usage/series（聚合 usage_log.jsonl 按日期,
  输入/输出/费用）; 前端 X=日期堆叠柱状图; 实测 6 天真实数据。
- **管道页**: 说明卡 + 运行模式（自适应/固定+步长）+ 参数变更记录。
- **深层链**: 说明卡逐 tab 解释。

### 验证
- 83 测全绿（新增 router 持久化 + usage series 测试）+ tsc 零错误 + build 成功
  + 4173 服务新构建 + 端点实测（路由 PUT、series 6 天真实数据）。

### 待办（FRONTEND_OVERHAUL_20260818.md 续）
- Mind 空间 / 元认知内页 / 图谱注释 / 网关日期范围+命中率 / 递归地图可视化 /
  管道副屏参数详情模块（含图表）/ 工程模块编辑做实。

## 十二、页面说明改「关于」悬浮（2026-08-18, 已提交本地）

- 新增 `frontend/src/components/ui/AboutPopover.tsx`（小「关于」按钮,
  悬停玻璃弹层, 不占页面空间）。
...
## 十七、网关压测全维度 + 性能优化（2026-08-20）

### 量化数据（switch docs/BENCH_20260820.md, mock 上游实测）
- 缓存命中: 4210 req/s（64 并发）/ 2837（256）, 命中率 99.5%, 命中 ~2ms
  vs 真实上游 ~1.08s。
- 未命中路径: 32/64/128/256 并发 = 2781/2873/1814/2780 req/s（0 失败）。
- 高并发稳定: 常驻 128 并发 10s = 26176 请求 / 2607 req/s / 0 失败 /
  平均延迟 48.8ms; 网关 RSS 稳定 ~29MB。
- 长上下文流式 SSE: 300 并发流 0 失败, 19500 事件完整（65/流）。
- 错误场景: 50% 429 注入 → 正确分类 + 熔断自动恢复（half_open→closed）。

### 优化与发现
- **已修**: usage_log 每请求 OpenFile+Close（全局锁内）→ 持久句柄+独立写锁。
- **待 profile**: 两条路径吞吐上限 ~2.8K req/s, 高并发 p50 升至 ~90ms,
  疑似每请求结构化日志全局锁（下一步对照关闭日志压测）。
- 压测工具修正: gwbench2 v1 stab 轮次屏障 140/s 伪影 → v2 常驻 worker 池
  （真实 2607 req/s）; 新增 mockupstream 错误/流式注入。
- WS 结论: 网关为 HTTP/SSE 形态（无 WS, 不需要）; DialogMesh 聊天 WS 在
  API 侧。

## 十八、网关日志根因定位 + 吞吐修复（2026-08-20）

### 根因（对照关日志压测定位）
- 每请求日志 = **两处 `log.Printf`**（MetricsMiddleware 结构化日志 +
  LoggingMiddleware 冗余）, 都走 log 包全局锁 → 高并发吞吐被压到 ~2.8K。
- 对照数据: 关日志 c=256 未命中 2780→9839 req/s（3.5×）。

### 修复（已推 GitHub, 保观测）
- 结构化日志改**异步批量写入**（channel 入队 + 50ms 攒批刷盘, 满则丢计数）;
- **移除冗余 LoggingMiddleware**; 加 `DM_GATEWAY_REQUEST_LOG=0` 开关（关
  每请求 Info, 错误日志始终保留）。
- 实测: 未命中 c=256 2780→**7189**（保日志）/ 9839（关日志）;
  缓存命中 c=256 2837→**25813**（p50 3ms, 0 失败）; c=64 14820。

### 20K 之谜答案
- 早期 22.8K 是缓存命中路径的理想数字; 现在的实现里每请求还有记账/异步
  日志开销, 优化后缓存命中回到 25.8K。
- c=512 高失败率（11-40%）是**压测客户端连接瓶颈**（网关日志全 200 无
  5xx）——与早期「20K 是客户端问题」一致; 网关本身在更高并发下无压力。
