...
class DomainObservation:
    ...
    confidence: float               # 该域 Observation 的整体置信度（非 Interpretatoin 置信度）
...
class Interpretation:
    ...
    confidence: float               # 初始置信度（来自 compiler，后续由 Hypothesis Engine 更新）
...
- **`confidence`** 由 Compiler 赋初始值，**Hypothesis Engine** 负责后续更新。Compiler 永远不修改已有 Interpretation
...
Interpretations:
  A: "调整中间件顺序"          (confidence=0.75)
  B: "优化请求处理性能"        (confidence=0.40)
  C: "遵循工程规范重排"        (confidence=0.25)
```
...
Interpretations:
  A: "手动编排节点"            (confidence=0.80)
  B: "探索性操作"              (confidence=0.35)
  C: "整理布局"                (confidence=0.30)
```
...
Interpretations:
  A: "请求修改 Pipeline 顺序"  (confidence=0.95)
  B: "测试 RateLimiter 效果"   (confidence=0.20)
```
...
- `obs.hypothesis.trigger_count` (default: 5)
- `obs.hypothesis.trigger_interval_ms` (default: 60000)
