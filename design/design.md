...
# DialogMesh v6 — 当前架构 (2026-07-27)

> 以下为 v6 运行时最新状态。上文 v3 设计保留为历史参考。

## 架构总览

```
用户输入 → v3_session_api
  ├── Phase 1: AgentOrchestrator 认知分析
  ├── Phase 2: 画像注入 (OCEAN/MBTI/BFI)
  ├── Phase 3: BlueprintEngine → BlueprintDAG
  │   └── SkillRegistry.match(intent) → TEMPLATE/HYBRID/LLM_DRIVEN
  ├── Phase 3.5: Decider → 3 Tick 并行
  │   Tick 0: pcr + intent (并行)
  │   Tick 1: context + profile (并行)
  │   Tick 2: llm_reply
  ├── Phase 4: switch Gateway → DeepSeek v4
  └── Phase 5: task_graph → 前端 TaskPlanningPage
```

## 10 链状态

✅ PCR / Discourse / Intent / MetaP / Profile / Blueprint
⚠️ Context / Behavior / Association / Meta
❌ Engineering / Subgraph

## 新增模块

- **Blueprint 编排** `core/agent/blueprint/` 8模块 ~1,500L — DAG构建/执行/约束/学习
- **学习摄入** `core/agent/learning/` 7模块 ~1,300L — 5源搜索/trafilatura提取/ChromaDB/SelfCheckGPT

## 存储

ChromaDB(外部) ──单向──→ 规则 ──→ EventLog ← Meta验证
HybridIndex(内部) — sessions/events/relations

## 监控

PipelineTracer → data/pipeline_traces.jsonl (人+Meta双用)
