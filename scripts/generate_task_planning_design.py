# -*- coding: utf-8 -*-
"""
scripts/generate_task_planning_design.py
──────────────────────────────────────
根据对话历史、代码快照和文献洞察，生成 DialogMesh 动态任务规划架构设计文档。
"""
import os

# 目标文档路径
TARGET = "docs/frontend-agent/DESIGN_TASK_PLANNING_DYNAMIC.md"

DOC = r"""# DialogMesh 动态任务规划架构设计 (Dynamic Task Planning v1.0)
...
def main(ctx):
    run_dir = ctx["runDir"]
    target_path = os.path.join(run_dir, os.path.basename(TARGET))
    ...
    # 同时尝试写入项目目录（如果可写）
    project_path = os.path.join(os.getcwd(), TARGET)
    try:
        ...
        return {
            "status": "success",
            "files": [target_path, project_path],
