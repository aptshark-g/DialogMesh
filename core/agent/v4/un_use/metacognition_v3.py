# -*- coding: utf-8 -*-
"""v3 元认知 Adapter — 已归档（M8 归一，2026-08-05，A17 记录不删）。

归档原因:
  v3 规则式 MetaCognitionAdapter + MetaCognitionScheduler 零生产调用
  （仅 subsystem_registrations 注册串引用），且与 v4 MetaCognition
  （v4/cognitive/metacognition.py，唯一内核）职责重叠。
  归一方向（M8 拍板）:
    - 内核 = v4 MetaCognition（审核队列/复盘/双模式/自我复盘）
    - 学习闭环接入 = v6 MetaConsumer（作为 v4 内核组件，consume_trace）
    - 写回通道 = meta/feedback_bridge.py（保留）
    - 冷路径触发 = meta/meta_subscriber.py（保留）
  v3 的 MetaCognitionScheduler（token 阈值触发）思想并入 v4 主动扫描策略，
  不保留独立类。
"""
