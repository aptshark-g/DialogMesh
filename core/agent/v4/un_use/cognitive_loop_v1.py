# -*- coding: utf-8 -*-
"""event/cognitive_loop.py — 已归档（M9 拍板，2026-08-05，A17 记录不删）。

归档原因:
  全文件（BehaviorLearner + MetaReviewer + CognitiveLoop + wire_cognitive_loop）
  零生产调用、零测试引用。MetaReviewer 与 v4 MetaCognition（唯一内核）职责重叠
  （其 review_chains API 在 v4 不存在，hasattr 保护下恒空转）；BehaviorLearner
  已被行为链 DPO 批次覆盖。保留本文件仅作历史参考。
  备注: v4/cognitive/runtime.run_cognitive_loop 是不同模块（活路径），不受影响。
"""
...
class BehaviorLearner:
    ...
    Process: CausalPlanner.process_chain() → WeightUpdater → saves to disk."""
    ...
    def learn_now(self) -> dict:
        ...
        # Trigger CausalPlanner causal chain processing if available
        cp = getattr(self._engine, '_causal_planner', None)
        if cp and hasattr(cp, 'process_chain'):
            try:
                chain_result = cp.process_chain()
                result["causal_path"] = True
                result["causal_triggered"] = bool(getattr(chain_result, "triggered", False))
                result["causal_updates"] = len(
                    getattr(chain_result, "edge_updates", []) or []
                )
            except Exception as e:
                logger.debug("CausalPlanner process_chain failed: %s", e)
