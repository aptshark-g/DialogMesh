...
def cmd_blueprint_show(args):
    ...
    if be:
        status = getattr(be, 'status', lambda: {})()
        print(json.dumps(status, indent=2, ensure_ascii=False, default=str))
        return
    from core.agent.blueprint.engine import BlueprintEngine
    be = BlueprintEngine()
    dag = be.build("show", intent="show", strategy="TEMPLATE")
...
def cmd_blueprint_build(args):
    ...
    print(json.dumps({"status": "building", "text": text}, ensure_ascii=False))


def cmd_blueprint_validate(args):
    from core.agent.blueprint.engine import BlueprintEngine
    be = BlueprintEngine()
    dag = be.build("validate", intent="validate", strategy="TEMPLATE")
...
def cmd_blueprint_export(args):
    from core.agent.blueprint.engine import BlueprintEngine
    be = BlueprintEngine()
    dag = be.build("export", intent="export", strategy="TEMPLATE")
...
def cmd_decider_show(args):
    ...
    if gd:
        ...
        return
    print(json.dumps({"error": "No decider available"}, ensure_ascii=False))
...
def cmd_decider_chains(args):
    ...
    if gd and hasattr(gd, 'stats'):
        s = gd.stats()
        print(json.dumps({"tick": s.get("tick", 0), "state": s.get("state", "idle")},
                         ensure_ascii=False))
        return
    print(json.dumps({"chains": [], "msg": "No decider"}, ensure_ascii=False))
