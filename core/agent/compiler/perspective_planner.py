...
class PerspectivePlanner:
    ...
    def _ensure_descriptions(self):
        ...
        if PerspectivePlanner._STRATEGY_DESCRIPTIONS is not None:
            return
        try:
            from core.agent.compiler.soft_config import load_perspective_config
    ...
    def _bge_select_strategy(self, text: str) -> str:
        ...
        try:
            from core.agent.compiler.soft_config import SoftConfig
        ...
        try:
            ...
            for strat, desc in self._STRATEGY_DESCRIPTIONS.items():
                # Append English description from soft_config
                if is_english:
                    try:
                        from core.agent.compiler.soft_config import SoftConfig
                        cfg = SoftConfig("intent_patterns.json", {}).load()
                        en = cfg.get("bge_english_descriptions", {}).get(strat, "")
                        if en: desc = desc + " " + en
                    except Exception:
                        pass
