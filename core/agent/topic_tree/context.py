...
from .heat_model import AdaptiveHeatModel
...
class DualPerspectiveContext:
    ...
    def touch(self, block_id: str):
        self.heat.touch(block_id)
    
    def on_correction(self, block_id: str):
        self.heat.on_correction(block_id)
    
    def on_branch_switch(self):
        self.heat.on_branch_switch()
    
    def user_channel(self, active_node: str, budget: int) -> List[Summary]:
        hot = self.heat.hot_nodes(limit=10)
        summaries = []
        for block_id in hot:
            if budget <= 0: break
    ...
    def system_channel(self, active_node: str, budget: int) -> List[Summary]:
        neighbors = []
        for key in self.relations._current:
            a, b = key.split("::")
            if a == active_node or b == active_node:
                other = b if a == active_node else a
                neighbors.append(other)
        critical = self.heat.topology_critical(active_node, neighbors)
        summaries = []
        for node_id in critical:
            if budget <= 0: break
            fact = self.facts.get(node_id)
            if fact:
                s = Summary(node_id=node_id, content=fact.content[:budget], layer=2,
                          token_count=min(len(fact.content), budget))
                summaries.append(s)
                budget -= s.token_count
        return summaries
...
class MultiPerspectiveBranchView:
    ...
    def register(self, module: str, block_id: str, branch_id: str, reason: str = "", **extra):
        self._perspectives.setdefault(block_id, {})[module] = {"branch": branch_id, "reason": reason, **extra}
    ...
    def get_view(self, block_id: str) -> dict:
        ...
        return {"block_id": block_id, "perspectives": views, "consensus": consensus,
                "summary": f"{len(views)} modules: {'all agree' if consensus else 'disagree on branch'}"}


class BehaviorDrivenRefresh:
    def __init__(self):
        self._dirty_blocks: Dict[str, str] = {}
        self._last_refresh: Dict[str, float] = {}
        self._local_model_available = False
    
    @property
    def local_model_available(self): return self._local_model_available
    
    def set_local_model(self, available: bool): self._local_model_available = available
    
    def on_correction(self, block_id: str, correction: str = ""):
        self._dirty_blocks[block_id] = f"correction: {correction[:50]}"
    ...
    def should_refresh(self, block_id: str, layer: int, ttl_rounds: int = 10) -> bool:
        if block_id in self._dirty_blocks: return True
        if layer == 1 and self._local_model_available: return True
        last = self._last_refresh.get(block_id, 0)
        if last > 0 and (time.time() - last) > ttl_rounds * 30: return True
        return False
