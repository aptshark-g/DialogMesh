...
LOCALE = {
    ...
    "tui.settings.lang": "语言: [{lang}] {name}",
    "tui.settings.press": "按 'e' 切换英文, 'z' 切换中文",
    "tui.settings.locale_count": "当前语言键数: {count}",
    "tui.settings.changed": "(更改在下次刷新时生效)",
    "tui.no_observations": "(暂无观测数据)",
    "tui.no_hypotheses": "(暂无活跃假说)",
    "tui.no_hypotheses_available": "(暂无假说数据)",
    "tui.no_knowledge": "(暂无冻结知识)",
    "tui.no_skills": "(暂无技能)",
    "tui.not_available": "(不可用)",
    "tui.graph.stats": "图: {world} ({nodes} 节点, {edges} 边)",
    "tui.world.top_backbone": "骨干节点:",
    "tui.world.communities": "社区 ({count} 个):",
    "tui.world.community_item": "  {cid}: {size} 个节点",
    "tui.world.usage_hint": "使用: engine._world_graph = CodeWorldAdapter().build_graph('.')",
    "tui.context.intent": "意图: {intent} ({total} 项)",
    "tui.context.send_hint": "通过 CLI 或 API 发送事件以触发编译。",
    "tui.context.domain_item": "  [{src}] {count} 项 (相关度: {rng})",
    "tui.context.domains": "域: {domains}",
    "tui.events.total": "总计: {total} 个事件, {unconsumed} 未消费",
    "tui.events.more": "... (还有 {count} 个事件)",
    "tui.events.no_db": "创建 data/ 目录或发送事件来填充。",
    "tui.events.not_available": "事件日志不可用: {error}",
    "tui.events.module_error": "事件日志模块不可用",
    "tui.error": "错误: {error}",
